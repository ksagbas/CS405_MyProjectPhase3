...
void main()
{
    vNormal = normalize(vec3(normalMV * vec4(normal, 0.0))); // Normalize edilmiş normaller
    vPosition = vec3(mv * vec4(position, 1.0));
    vFragPos = vec3(modelMatrix * vec4(position, 1.0)); // Fragman pozisyonu
    vTexCoord = texCoord;
...
void main()
{
    ...
    vec3 lightDir = normalize(lightPos - vFragPos); // Normalize the light direction
    ...
    vec3 viewDir = normalize(-vFragPos); // Görüş yönü
    ...
    if (isLightSource) {
        gl_FragColor = texture2D(tex, vTexCoord) * vec4(ambient, ambient, ambient, 1.0); // Dokuyu korur
    } else {
